"""
sample script showing full flow of reserving sandbox, running robot test, passing in sandbox id and other data
"""
import json
import time
...
import robot  # pip install robotframework
...
def run_robot_test(api: SandboxRestApiSession, sandbox_id: str, target_model: str):
    print("Starting Test!")
    ...
    robot_test_variables = [f"sandbox_id:{sandbox_id}",
                            f"resource_ip:{dut_component['address']}"]
    robot.run("robot_test.robot", variable=robot_test_variables)
...
if __name__ == "__main__":
    ...
    run_robot_test(api, sb_id, TARGET_DUT_MODEL)
